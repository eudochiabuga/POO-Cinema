...
//
// Implementeaza ICinemaService (interfata cu metode pure virtuale)
// pentru a demonstra polimorfismul si principiul segregarii interfetelor.
...
public:
    ...
    ~Cinematograf() override = default;       // toate repository-urile au detineMemoria=true

    // ---- ICinemaService - metode pure virtuale implementate ----
    // Filme
    void AdaugaFilm(Film* f) override;
    bool StergeFilm(int id) override;
    Film* GasesteFilm(const string& titlu) const;
    Film* GasesteFilmDupaID(int id) const override;
    const Repository<Film>& GetFilme() const { return filme; }

    // Sali
    void AdaugaSala(Sala* s) override;
    bool StergeSala(int id) override;
    Sala* GasesteSala(const string& numar) const;
    Sala* GasesteSalaDupaID(int id) const override;
    ...
    Spectacol* CreeazaSpectacol(Film* f, Sala* s,
        ...
        double pret = -1) override;     // -1 = foloseste PretBaza
    bool StergeSpectacol(int id) override;
    Spectacol* GasesteSpectacol(int id) const override;
    ...
    // Rezervari
    Rezervare* FaRezervare(Persoana* p, Spectacol* sp, int rand, int col) override;
    Rezervare* FaRezervare(Persoana* p, Film* f, Sala* s, int rand, int col);
    bool StergeRezervare(int id) override;
    Rezervare* GasesteRezervare(int id) const override;
    ...
    void AfisareFilme() const override;
    void AfisareSpectacole() const override;
    void AfisareRezervari() const override;
